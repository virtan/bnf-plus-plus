...
%c::size_type %c::eoi(size_type base)
{
%d    std::cerr << "eoi(" << base << ") = " << (base<end?-1:0) << std::endl;
    return base<end?npos:0;
}

%c::size_type %c::compare_symbol(size_type base, unsigned char symbol)
{
    size_type res;
    if(base>=end)
        res=npos;
...
%c::size_type %c::compare_range(size_type base, unsigned char since, unsigned char till)
{
    size_type res;
    if(base>=end)
        res=npos;
...
%c::size_type %c::compare_string(size_type base, const char *str)
{
...
%c::size_type %c::find_string(size_type base, const char *str)
{
...
%{
%c::size_type %c::%f(size_type base)
{
